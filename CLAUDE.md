...
- `tipos_evento` — catalog of disease/event types: `dengue` (piloted, live in the panel) and `ira` (ADR 0011, accepted 2026-08-22 — catalog row + `clasificacion` schema support only, no rows in `casos_epidemiologicos` yet, no live UI beyond the exploratory `/ira` page).
