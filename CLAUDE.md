...
- **M3 — Presión epidemiológica relativa** — **not implemented, no approved formula.** Intended to compare observed cases against a historical reference (percentile-based), taking over the retired classifier's descriptive role without turning back into a risk label. Do not invent the formula unilaterally.
...
M1/M2 are served on request via `GET /api/v1/spatial/current?week=&year=` and `GET /api/v1/temporal/{departamento_codigo}?anio=` — **no schema changes**, nothing persisted for either module. The map's layer selector (`web/src/components/MapaDepartamentos.astro`) has working buttons for these two layers and disabled placeholders for M3/M4.
...
- **M3 (presión epidemiológica relativa) formula** — undecided. Likely built on the same endemic-channel percentile logic already validated for the retired national classifier (historical percentile within each department/region, never a population-incidence threshold — the population denominator for the training window was invalidated by the 2024 census), but that is not a closed decision, just a plausible starting point. Do not invent cuts, base-year scheme, neighbouring-week window, or sufficiency floor unilaterally.
...
- **Where M3/M4 output lives** — M1/M2 are computed on request with nothing persisted (`GET /api/v1/spatial/current`, `GET /api/v1/temporal/{departamento_codigo}`); whether M3/M4 follow the same pattern or need a table is undecided, and the schema has no table for any of this today.
