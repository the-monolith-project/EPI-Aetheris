...
IMPORTANTE: reproduce el esquema retrospectivo leave-one-year-out de los
informes historicos. No implementa la Via -1 ni debe presentarse como una
validacion prospectiva limpia: el dataset legado etiqueta todas las filas
antes de separar train/test y cada entrenamiento incluye anos posteriores al
ano evaluado. Su funcion es auditar resultados existentes y servir de control
antes de implementar el protocolo nuevo en un script experimental separado.
...
def imprimir_confusion(y_real, y_pred, titulo):
    # Se fijan siempre las tres clases para que un soporte cero no cambie la
    # forma de la matriz ni oculte falsos positivos.
    presentes = CLASES
...
def evaluar(filas, anio_prueba, cols, modelo=None, semilla=SEMILLA_REFERENCIA):
    """Reproduce el split retrospectivo legado; no es un fold prospectivo."""
    ...
    return {
        ...
        "f1_macro_modelo": float(f1_score(
            y_te, pred, labels=CLASES, average="macro", zero_division=0
        )),
        "f1_macro_base": float(f1_score(
            y_te, base, labels=CLASES, average="macro", zero_division=0
        )),
...
def linea_resumen(res, etiqueta):
    if res["recall_alto_modelo"] is None:
        return ("  %-14s F1 %.3f vs base %.3f | recall alto N/A "
                "(0 semanas 'alto' reales; evaluar falsos positivos)"
...
def main():
    ...
    entorno = {
        ...
        "seed_sql_sha256": hashlib.sha256(args.seed_sql.read_bytes()).hexdigest(),
    }
    ...
    parametros = {
        ...
        "protocolo_evaluacion": "retrospectivo_legado_leave_one_year_out",
        "advertencia_validacion": (
            "Reproduce informes historicos; no implementa la Via -1 prospectiva."
        ),
    }
    ...
    print("   Un umbral bajo puede aumentar recall, pero el criterio exige")
    print("   superar simultaneamente F1 macro y recall. AUC < 0,5 indica")
    print("   que el ordenamiento de P(alto) esta invertido en estas pruebas.")
    print("=" * 78)
    resultados["auc"] = {}
    ...
    for anio in [2019, 2022]:
        ...
        # Barrido descriptivo sobre el test. No selecciona un umbral desplegable:
        # cualquier seleccion valida debe ocurrir dentro de los datos de train.
        ...
        for t in [0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50]:
            ...
            f1m = f1_score(
                res["_y_te"], pred_t, labels=CLASES,
                average="macro", zero_division=0
            )
