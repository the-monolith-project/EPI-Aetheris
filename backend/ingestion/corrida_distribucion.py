...
# ⚡ Bolt Optimization: Replacing statistics.quantiles and statistics.median
# with custom inline math functions. The standard library's statistics module
# introduces massive overhead in tight data aggregation loops due to exact fraction representation.
# This custom implementation relies on standard list sorting and basic math, significantly reducing processing time.
def _percentil(valores: list[float], p: float) -> float:
    """Interpolacion lineal, metodo 'inclusive' generalizado a un percentil p arbitrario."""
...
def _mediana(valores: list[float]) -> float:
    ...
    n = len(ordenados)
    mid = n // 2
    return (ordenados[mid - 1] + ordenados[mid]) / 2.0 if n % 2 == 0 else ordenados[mid]
    ...
    for depto in DEPARTAMENTOS:
        for semana in range(1, 53):
            ...
            for anio_objetivo in ANIOS:
                ...
                if len(valores) >= 2:
                    q25 = _percentil(valores, 0.25)
                    q50 = _mediana(valores)
                    q75 = _percentil(valores, 0.75)
                elif len(valores) == 1:
