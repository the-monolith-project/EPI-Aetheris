...
# ⚡ Bolt Optimization: Replaced statistics.mean, statistics.median, and statistics.stdev
# with custom inline math functions. The standard library's statistics module
# introduces massive overhead in tight data aggregation loops. This optimization uses standard operations
# and math.sqrt to achieve the same result significantly faster.
def chequeo_de_cordura(serie_iv: dict[str, dict[int, dict[int, float]]]) -> list[float]:
    ...
    mid = n // 2
    mediana = (
        (ordenados[mid - 1] + ordenados[mid]) / 2.0 if n % 2 == 0 else ordenados[mid]
    )
    varianza = sum((x - media) ** 2 for x in todos) / (n - 1) if n > 1 else 0.0
...
def calcular_alertas_por_anio(
    ...
    for anio_obj in anios_evaluar:
        ...
        for semana in semanas_anio:
            ...
            mid = n_pool // 2
            mediana = (
                (ordenados[mid - 1] + ordenados[mid]) / 2.0
                if n_pool % 2 == 0
                else ordenados[mid]
            )
            media = sum(pool) / n_pool
            varianza = (
                sum((x - media) ** 2 for x in pool) / (n_pool - 1)
                if n_pool > 1
                else 0.0
            )
            desv = math.sqrt(varianza)
