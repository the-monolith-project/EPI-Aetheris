...
## 2026-08-29 - ResizeObserver Observable Plot re-rendering bottleneck
**Learning:** ResizeObservers calling render functions containing heavy chart plotting (`@observablehq/plot`) can severely degrade client-side performance. On a window resize event, the observer fires repeatedly, causing redundant synchronous recalculations and DOM manipulations which block the main thread.
