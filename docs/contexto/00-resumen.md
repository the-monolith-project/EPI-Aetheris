...
EPI-Aetheris es un sistema **open-source, contenedorizado y desplegable a costo cero** que cruza datos públicos históricos de casos de dengue con variables climáticas, piloteado en El Salvador. **El objetivo original — clasificar el riesgo de brote (alto/medio/bajo) por semana epidemiológica — quedó cerrado el 2026-08-18** (`docs/rescate-prediccion/informe-cierre-rescate-prediccion.md`): cinco vías de validación no sostuvieron capacidad predictiva real en los años con brote conocido. El proyecto pivotó a una herramienta de análisis espacio-temporal descriptiva en su base, organizada en cuatro módulos (M1 idoneidad biofísica, M2 anomalía climática continua, M3 presión epidemiológica relativa e M4 integridad de vigilancia — los cuatro implementados; M4 son tres métricas de calidad del dato, no un índice de riesgo). Ver el pivote del 2026-08-18 en `01-decisiones-cerradas.md` para el detalle completo. **Sobre esa base descriptiva hay desde el 2026-09-10 una capa predictiva acotada y autorizada** (nowcast, ADR 0020): la pestaña "Predicción a corto plazo" de `/dengue` estima el conteo semanal de la serie nacional agregada a 1–8 semanas desde la última semana observada, con intervalo calibrado y desempeño a la vista. No es el clasificador retirado de vuelta: no hay etiqueta alto/medio/bajo, no hay salida departamental y el horizonte va rotulado. El mapa sigue pintando datos departamentales de MINSAL como capa descriptiva — nunca como salida de un clasificador, porque ya no hay ninguno en producción. La arquitectura es agnóstica al tipo de evento y a la región — dengue/El Salvador es el caso piloto, no el límite.

**Lo que NO es:** ni un descubrimiento científico ni "predecir brotes de dengue con IA". El intento de clasificar riesgo de brote se cerró sin adoptarse (2026-08-18); lo que sí existe es una predicción estadística de **horizonte corto** sobre el conteo nacional agregado, validada con protocolo predeclarado y confirmación independiente, con su alcance declarado junto a la cifra (ADR 0020). La frontera importa: se predice un **conteo a semanas**, no "si habrá brote". El aporte es de **ingeniería de software**: entregar como sistema reproducible y gratuito una capa de integración, trazabilidad y exploración histórica que la academia no publica como software. Ver `01-decisiones-cerradas.md` para el posicionamiento frente al estado del arte.
...
- Repositorio: `github.com/the-monolith-project/EPI-Aetheris`, rama principal `main` con protección activa (PR obligatoria; `dev` es la rama de integración).
- **Segundo repositorio:** `github.com/the-monolith-project/aetheris-nitor` — copia independiente del frontend Astro, desplegada en su propio dominio `epi-aetheris.dev` contra el mismo backend. Ha divergido de `web/`. Ver "Segundo frontend" en `01-decisiones-cerradas.md` antes de tocar cualquier componente compartido.

## El problema

El dengue es endémico en El Salvador. La respuesta institucional es mayoritariamente reactiva pese a que existe correlación científicamente documentada entre incidencia y clima (temperatura, lluvia, humedad) con semanas de rezago. Falta una herramienta local, desplegable y gratuita que cruce casos con clima de forma sistemática. Público objetivo: unidades de epidemiología y tomadores de decisión que priorizan fumigación, campañas y recursos. **Dimensión concreta que el informe mide desde el 2026-09-23:** la información de vigilancia está dispersa en boletines PDF semanales con cifras acumuladas, así que obtener el dato semanal de un departamento exige abrir dos boletines y restar. La hipótesis del Capítulo III contrasta el tiempo y la exactitud de esa consulta manual contra el tablero — ver "Hipótesis y variables del informe de investigación" en `01-decisiones-cerradas.md`.
