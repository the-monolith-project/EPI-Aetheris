...
**E. Generado con `--disable-triggers` y post-procesado para quitar esos toggles.** `regiones` tiene una referencia a sí misma (departamento → país vía `region_padre_id`) que `pg_dump` señala como advertencia de "restricción circular"; `--disable-triggers` evita que la carga falle por orden de FK durante el `COPY`. No aplica en la práctica a este volcado (esa tabla está excluida — ver punto B), pero se deja la bandera por si el volcado vuelve a incluir alguna tabla con auto-referencia en el futuro.

**E (enmienda 2026-09-06, issue #82).** El volcado versionado **no** incluye las líneas `ALTER TABLE ... DISABLE/ENABLE TRIGGER ALL` que emite `pg_dump --disable-triggers`. En el Postgres gestionado de Render el rol `aetheris_user` no es superuser, y esas líneas abortan la carga (`permission denied: "RI_ConstraintTrigger_..." is a system trigger`) con rollback de toda la transacción. El generador `db/generar_seed.sh` post-procesa la salida con `grep -vE 'DISABLE TRIGGER ALL;|ENABLE TRIGGER ALL;'`. El orden de tablas por dependencia (catálogos por migración → boletines → semanas → casos → variables → vigilancia) ya garantiza las FK sin desactivar triggers. Regenerar el seed es seguir usando ese script, no `pg_dump ... > seed_datos_reales.sql` a pelo.
...
**G. No se automatiza la regeneración.** El volcado es una foto de un momento (original: 2026-08-17; regenerado 2026-09-01 para incluir la ingesta respiratoria; saneado 2026-09-06 para quitar los toggles de trigger, issue #82); no se recalcula en cada commit ni hay hook que lo mantenga sincronizado con la base de desarrollo. Regenerarlo es una acción manual (`db/generar_seed.sh`, mismos flags que este ADR documenta más el post-proceso del punto E) cuando el equipo decida que vale la pena una foto más reciente — no antes de cada PR.
