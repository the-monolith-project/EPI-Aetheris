...
**El pin de versión está verificado contra el contenido, no solo afirmado.** `boundaryID` (`SLV-ADM1-98794003`) no aparece como campo suelto dentro del GeoJSON, pero su componente numérico sí: las 14 features de `slv-adm1-source.geojson` tienen `shapeID` con prefijo `98794003`, `shapeGroup` `"SLV"` y `shapeType` `"ADM1"` — confirmado por inspección directa el `2026-08-07`. `build_geo_departamentos.py` ahora comprueba esto en cada corrida (`_verificar_boundary_pineado`, antes de unir) y falla con `sys.exit(1)` si el archivo fuente deja de corresponder al boundary pineado — por ejemplo si alguien lo reemplaza por otro país, otro nivel admin, o una versión cuyo esquema de `shapeID` cambió. El pin queda cerrado: no es solo texto del ADR, es un invariante que el build revisa.
...
* Positivo: el pin de versión (`BOUNDARY_ID`) también se verifica en cada build contra `shapeID`/`shapeGroup`/`shapeType` del archivo fuente, no solo se declara — probado deliberadamente (pineando a un `BOUNDARY_ID` inexistente) para confirmar que también dispara.
...
* Negativo: la unión por nombre sigue siendo más frágil que un código real; el catálogo de 14 pares está hardcodeado en el script como espejo de la migración — mitigado por un test que lo compara contra `0001_init_schema.sql` y falla explícitamente si extrae un número de filas distinto de 14 (no solo si los conjuntos difieren), pero sigue siendo edición manual, no lectura automática de una única fuente.
...
* Negativo: actualizar el boundary a una versión más nueva de geoBoundaries sigue siendo un paso manual (reemplazar `slv-adm1-source.geojson`, actualizar `BOUNDARY_ID` si cambió, repetir la verificación 14-a-14, actualizar este ADR) — la verificación automática evita que el build *use* silenciosamente un archivo que no corresponde al pin, pero no fuerza a nadie a *hacer* la actualización correctamente cuando sí es intencional.
