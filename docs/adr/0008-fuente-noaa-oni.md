...
**B. Nueva variable:** `oni_anom` en `variables_ambientales.variable` (catálogo de texto libre, sin `CHECK` — ver `AGENTS.md`, actualizado en la misma sesión que este ADR). Valor = anomalía de temperatura superficial del mar (ANOM), no el valor absoluto (TOTAL) — es la anomalía la que se usa operacionalmente para clasificar fase El Niño/Neutral/La Niña.
